...
describe('toWireDescriptor (#3317 — PO 확定 wire 형상)', () => {
  ...
  test('실 커넥터 정본(threads·stibee·instagram) 전부 — requiresEnv 이름이 fields에 중복 선언되지 않는다(비밀 유출 0 구조 증명)', () => {
  ...
  test('⭐페드루 요청(디디군 PR B 그라운딩) — wire 최상위 kinds가 실제 배선된 능력과 일치한다: threads=publish+measure(#3321), stibee=publish만, instagram=publish만(story a98dfbea)', () => {
...
describe('__fixtures__ 픽스처 드리프트 pin (#3317 PR#31 — 디디군 백엔드 PR A 파리티 테스트용)', () => {
  ...
  test('instagram.content-package.json == toWireDescriptor(INSTAGRAM_CONNECTOR_DESCRIPTOR) — connectors/__fixtures__/instagram.content-package.json(story a98dfbea)', async () => {
    const fixture = await Bun.file(new URL('./__fixtures__/instagram.content-package.json', import.meta.url)).json()
    expect(fixture).toEqual(toWireDescriptor(INSTAGRAM_CONNECTOR_DESCRIPTOR))
  })
})
